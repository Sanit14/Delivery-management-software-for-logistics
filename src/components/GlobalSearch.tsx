    ...
    const search = async () => {
      ...
      firms
        ...
        .forEach(f => out.push({
          ...
          link: `/info/firm/${encodeURIComponent(f.value)}`,
        }));
      ...
      // BS numbers (DRs) — year-aware quick match: typing "47" finds this year's
      // "26-00047" (current year assumed; full old numbers still searchable).
      // Selecting one opens the quick preview popup, same as LR search.
      const allDrsForBs = await db.drs.filter(d => !d.deletedAt).toArray();
      bsMatches(allDrsForBs, query, 5).forEach(d => out.push({
        type: 'bs', label: `BS ${d.drNumber}`, sub: d.consignee,
        link: '', drId: d.id,
      }));

      // LR numbers (DRs) — selecting one opens a quick preview popup (for phone
      // enquiries), with a button to jump to the full memo. LR is unique.
      const lrMatches = await db.drs.filter(d => !d.deletedAt && d.lrNumber.toLowerCase().includes(q)).limit(5).toArray();
      lrMatches.forEach(d => out.push({
        type: 'lr', label: `LR ${d.lrNumber}`, sub: `${d.consignee} · BS ${d.drNumber}`,
        link: '', drId: d.id,
      }));
