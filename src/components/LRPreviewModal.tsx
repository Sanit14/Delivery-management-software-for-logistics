...
export default function LRPreviewModal({ drId, onClose, heading = 'lr' }: { drId: number; onClose: () => void; heading?: 'lr' | 'bs' }) {
  ...
  // Entry date lives on the parent challan (set on its header page), not the
  // DR — bookingDate is the only date already on the DR record itself (set on
  // the details/LR-entry page).
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setDr(d || null);
      if (d) {
        const challan = await db.challans.get(d.challanId);
        if (!alive) return;
        setEntryDate(challan?.entryDate || '');
        ...
        <div style={{ padding: '12px 16px', display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 8 }}>
          ...
          <div style={{ background: 'var(--border-light)', borderRadius: 8, padding: '8px 10px', minWidth: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
              <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Booking</span>
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>{dr.bookingDate ? formatDate(dr.bookingDate) : '—'}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginTop: 3 }}>
              <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Entry</span>
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>{entryDate ? formatDate(entryDate) : '—'}</span>
            </div>
          </div>
        </div>
