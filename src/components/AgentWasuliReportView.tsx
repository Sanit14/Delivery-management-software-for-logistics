...
interface WasuliRow {
  ...
  assignedDate?: string; collectedDate?: string; drNumber?: string; lrNumber?: string;
  consigneeName?: string; consignor?: string; challanNumber?: string; notes?: string;
}

const TYPES: { id: ReportType; label: string }[] = [
  { id: 'assigned', label: 'Assigned' }, { id: 'pending', label: 'Pending' },
 ...
 * Loads every (non-deleted) wasuli row up front — unlike the Wasuli
 * operational page, which only ever loads assigned+collected rows, this needs
 * the unassigned pool too for the "Pending" status. buildWasuliReport (the one
 * wasuli-report engine) does the grouping; WasuliReportPrint (the one layout)
 * renders it — visibly here, and identically for Print/PDF/CSV.
 */
    ...
    (async () => {
      ...
      setRows(all.map(w => {
        ...
        return {
          ...
          assignedDate: w.assignedDate, collectedDate: w.collectedDate, notes: w.notes,
