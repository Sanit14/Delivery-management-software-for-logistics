...
export default function FirmChallanReport() {
  ...
  const load = useCallback(async () => {
    ...
    setSummary({
      firmName, companyName: cn?.value || 'Sundeep Freight Movers',
...
function FirmChallanSummaryView({ data, onPrint, savedAdjustments, onSaved }: {
  ...
  const freight = r.toPay + r.paid;
  const dcAmount = r.dcAmount; // this challan's own commission, already computed in buildChallanSummaryRow
  // PF is money owed TO the firm — a positive value ADDS to the firm balance,
  // same convention as the ledger (see createLedgerEntries) and the Firm Report.
  // It's entry-level only (Σ lrEntries.pf, via buildChallanSummaryRow) — never
  // separately editable here.
  const pfValue = r.pf;
  const baseNet = freight - dcAmount - r.truckHire - r.crossing - r.doorDelivery + pfValue - r.refund;
  ...
  const savedTotal = savedAdjustments.reduce((s, a) => s + a.sign * a.amount, 0);
  const editingTotal = rows.reduce((s, a) => s + a.sign * (a.amount || 0), 0);
  const net = baseNet + (editing ? editingTotal : savedTotal);
  ...
  return (
    <div className="card" style={{ padding: 20 }}>
      ...
      <div style={{ borderTop: '1px solid var(--border)', paddingTop: 10 }}>
        {line('Freight (ToPay + Paid)', freight)}
        {line(`less Delivery Charges (${data.dcPercent}%)`, -dcAmount)}
        {line('less Truck Hire', -r.truckHire)}
        {line('plus PF', pfValue)}
        {r.crossing > 0 && line('plus Crossing', r.crossing)}
        {r.doorDelivery > 0 && line('plus Door Delivery', r.doorDelivery)}
        {r.refund > 0 && line('less Refund', -r.refund)}
