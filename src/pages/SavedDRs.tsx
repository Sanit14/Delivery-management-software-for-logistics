...
// A BS/LR number search should surface the matching document first, even when
// the broader firm/consignor/consignee/challan filter below also caught it and
// would otherwise leave it sitting wherever plain DR-number order puts it.
// Same identifier-priority rule as the Wasuli assign/collect boxes (bsMatch.ts).
const prioritizeStrongMatches = (candidates: DR[], q: string, ordered: DR[]): DR[] => {
  if (!q) return ordered;
  const strong = bsMatches(candidates, q, candidates.length, true);
  if (strong.length === 0) return ordered;
  const strongIds = new Set(strong.map(d => d.id));
  return [...strong, ...ordered.filter(d => !strongIds.has(d.id!))];
};
...
export default function SavedDRs() {
  ...
  const directMatches = useMemo(() => {
    ...
    return prioritizeStrongMatches(list, search, applySortDir(list));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const FirmCard = ({ firm }: { firm: string }) => {
    ...
    const firmDRs = useMemo(() => {
      ...
      return prioritizeStrongMatches(list, innerQuery, applySortDir(list));
      // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    <div>
        ...
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) setPreviewList(null); }}>
          <div style={{
            ...
            <div style={{ overflowY: 'auto', flex: 1, padding: 16, background: '#e8e8e8' }}>
              <PrintDRPage drs={previewList} companyName={COMPANY_NAME} />
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'none' }}>
        <PrintDRPage ref={printRef} drs={printDRs} companyName={COMPANY_NAME} />
      </div>
    </div>
  );
}
