...
import { Building2, Database, Trash2, Download, Upload, Truck, ShieldCheck, Info, History, Merge, Monitor } from 'lucide-react';
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setCompanyName(cn?.value || '');
      setCompanyLocation(cl?.value || '');
  ...
  const startRestorePreview = async (path: string) => {
    ...
    if (res.needsConfirm) {
      ...
      showToast(res.error || 'Restore fail hua', 'error');
    }
  };
  ...
  const confirmRestore = async (force: RestoreForce) => {
    ...
    if (res.ok) {
      ...
      showToast(('error' in res && res.error) || 'Restore fail hua', 'error');
    }
  };
  ...
  const saveCompany = async () => {
    await db.settings.bulkPut([
      { key: 'companyName', value: companyName.trim() },
      { key: 'companyLocation', value: companyLocation.trim() },
    ]);
  ...
  return (
    <div style={{ maxWidth: 680 }}>
      ...
      {section(<Building2 size={16} />, 'Company Details', (
        <>
          <div className="form-group">
          ...
          <button className="btn btn-primary btn-sm" onClick={saveCompany}>Save</button>
        </>
      ))}
      ...
      {section(<Merge size={16} />, 'Clean up duplicates', (
        <>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12, background: 'var(--border-light)', borderRadius: 'var(--r-md)', padding: '10px 14px', lineHeight: 1.7 }}>
          ...
          {isAdmin ? (
            ...
            <div style={{ fontSize: 13 }}>
              <button className="btn btn-outline btn-sm" onClick={() => requireAdmin(() => { /* unlock; content shows on re-render */ })}>
                <ShieldCheck size={14} /> Unlock (admin) to clean up duplicates
              </button>
            </div>
          )}
        </>
          ...
          <div className="confirm-modal" style={{ width: 500, maxWidth: '95vw' }}>
            ...
            <div className="confirm-body">
              ...
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                Fixed Charges (₹) — entry select hone par auto-fill honge
              </div>
              ...
              <div style={{ marginTop: 16, background: 'var(--navy)', borderRadius: 'var(--r-md)', padding: '12px 16px' }}>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', marginBottom: 8, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  Preview — Agar quantity = 10 dag ho
                </div>
