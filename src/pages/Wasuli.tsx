...
import { Check, UserCog, XCircle, Plus, RotateCcw, ArrowRight, Pencil, UserPlus } from 'lucide-react';
...
export default function Wasuli() {
  ...
  const [unassigned, setUnassigned] = useState<WRow[]>([]);
  const [collected, setCollected] = useState<WRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [tab, setTab] = useState<'unassigned' | 'assigned' | 'collected'>('assigned');
  ...
  const [skipRemarkPref, setSkipRemarkPref] = useState(false);
  ...
  const joinDRs = async (ws: Wasuli[]): Promise<WRow[]> => {
    ...
    return ws.map(w => {
      ...
      return {
        ...
        consignor: d?.consignor || '', consigneeName: d?.consignee || w.consigneeName, bookingDate: d?.bookingDate || '',
      };
    });
  };
  ...
  const load = useCallback(async () => {
    ...
    setUnassigned(await joinDRs(all.filter(w => w.status === 'pending' && w.operatorId === 0)));
    ...
    const skip = await db.settings.get('wasuliSkipRemarkPrompt');
    setSkipRemarkPref(skip?.value === 'true');
  ...
  const doAssign = async (picked?: string) => {
    ...
    await db.wasuli.update(w.id!, { operatorId: agentId, assignedDate: todayISO(), assignedAt: new Date().toISOString() });
  ...
  // One-click assign straight from the Unassigned tab — same write as doAssign,
  // skipping the BS lookup since the row is already in hand.
  const quickAssign = async (r: WRow) => {
    if (!agentId) { flash('Please select an agent first', 'error'); return; }
    await db.wasuli.update(r.id!, { operatorId: agentId, assignedDate: todayISO(), assignedAt: new Date().toISOString() });
    await logAudit({ action: 'assign', drNumber: r.drNumber, party: r.consigneeName, amount: r.amountToCollect, toAgent: operators.find(o => o.id === agentId)?.name });
    flash(`${r.drNumber} — ${r.consigneeName} assigned`, 'ok');
    load();
  };

  // Opens the remark editor — either right after a fresh collection (pre-filled
  // blank), or from "Add/Edit remark" on an already-collected row (pre-filled
  // with whatever's saved). Same modal, same handler, both callers.
  const openRemark = (r: WRow) => {
    setRemark(r.notes || '');
    setCollectTarget(r);
  };

  // Collection itself is instant — no modal, no confirm step, same shape as
  // doAssign. No stale-data guard needed here (unlike the old confirmCollect):
  // this runs immediately after validation in the same call, so there's no
  // window for another seat to have changed the row in between.
  const finishCollect = async (r: WRow) => {
    await db.wasuli.update(r.id!, {
      status: 'collected', collectedDate: todayISO(), collectedAt: new Date().toISOString(),
      collectedAmount: r.amountToCollect,
    });
    await logAudit({ action: 'collect', drNumber: r.drNumber, party: r.consigneeName, amount: r.amountToCollect, toAgent: operators.find(o => o.id === r.operatorId)?.name });
    flash(`${r.drNumber} — ${r.consigneeName} collected`, 'ok');
    load();
    // Purely an offer to attach a remark — collection is already saved. Stops
    // appearing for every seat once "don't ask every time" is checked below.
    if (!skipRemarkPref) openRemark({ ...r, collectedAmount: r.amountToCollect, notes: '' });
  };
  ...
  const doCollectByBS = async (picked?: string) => {
    ...
    setCollectBs('');
    await finishCollect(wrow);
  };

  const saveRemark = async () => {
    if (!collectTarget?.id) return;
    await db.wasuli.update(collectTarget.id, { notes: remark });
    await logAudit({ action: 'edit', drNumber: collectTarget.drNumber, party: collectTarget.consigneeName, amount: collectTarget.collectedAmount ?? collectTarget.amountToCollect, detail: remark || undefined });
  ...
  const doReassign = async () => {
    ...
    await db.wasuli.update(reassignTarget.id, { operatorId: reassignOp, assignedDate: todayISO(), assignedAt: new Date().toISOString() });
  ...
  const doCancel = async () => {
    ...
    await db.wasuli.update(cancelTarget.id, { operatorId: 0, assignedDate: '', assignedAt: '' });
  ...
  const doUncollect = async () => {
    ...
    await db.wasuli.update(uncollectTarget.id, { status: 'pending', collectedDate: CLEAR, collectedAt: CLEAR, collectedAmount: CLEAR });
  ...
  // Oldest-first by the real action time (assignedAt/collectedAt); falls back to the
  // day-only date for rows recorded before those timestamps existed.
  const myAssigned = assigned.filter(r => r.operatorId === agentId)
    .sort((a, b) => (a.assignedAt || a.assignedDate).localeCompare(b.assignedAt || b.assignedDate));
  const myCollected = collected.filter(r => r.operatorId === agentId)
    .sort((a, b) => (a.collectedAt || a.collectedDate || '').localeCompare(b.collectedAt || b.collectedDate || ''));
  // Same pool under every agent — unassigned rows don't belong to anyone yet.
  // Oldest booking first, so the longest-waiting DR surfaces first.
  const unassignedSorted = [...unassigned].sort((a, b) => (a.bookingDate || '').localeCompare(b.bookingDate || ''));
  ...
  const list = tab === 'assigned' ? myAssigned : tab === 'collected' ? myCollected : unassignedSorted;
  ...
  const renderRow = (r: WRow) => {
    const days = daysDiff((tab === 'unassigned' ? r.bookingDate : r.assignedDate) || '');
    return (
      <tr key={r.id}>
        <td className="td-bold">{r.drNumber}</td>
        <td>
           {r.consigneeName}
           {tab === 'assigned' && trails[r.drNumber] && trails[r.drNumber].length > 1 && (
        ...
        <td>{tab === 'collected'
          ...
          : <span style={dayPill(days)}>{days === 0 ? 'Today' : `${days}d`}</span>}</td>
        <td style={{ textAlign: 'right', fontWeight: 700, color: 'var(--orange)' }}>
          {formatINR(tab === 'collected' ? (r.collectedAmount || 0) : r.amountToCollect)}
        </td>
        <td>
          {tab === 'assigned' ? (
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              <button title="Collected" className="btn-icon" style={{ background: 'var(--success-bg)', color: 'var(--success)' }} onClick={() => finishCollect(r)}><Check size={14} /></button>
            ...
            <button className="btn btn-primary btn-sm" onClick={() => quickAssign(r)}><UserPlus size={14} /> Assign</button>
          ) : (
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              <button title={r.notes ? 'Edit remark' : 'Add remark'} className="btn-icon" onClick={() => openRemark(r)}><Pencil size={14} /></button>
  ...
  const tableHead = (
    <thead><tr>
      ...
      <th>{tab === 'collected' ? 'Collected' : tab === 'unassigned' ? 'Booking' : 'Assigned'}</th>
  ...
  return (
    <div style={{ paddingBottom: 60 }}>
      ...
      {loading ? (
        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
            ...
            <div className="stat-card" style={{ padding: '12px 14px' }}>
              <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>DRs unassigned</div>
              <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--orange)' }}>{unassignedSorted.length}</div>
            </div>
            <div className="stat-card" style={{ padding: '12px 14px' }}>
              <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>DRs assigned</div>
              <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--orange)' }}>{myAssigned.length}</div>
            </div>
            <div className="stat-card" style={{ padding: '12px 14px' }}>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 10, borderBottom: '2px solid var(--border)' }}>
            {(['unassigned', 'assigned', 'collected'] as const).map(t => (
              <button key={t} onClick={() => setTab(t)} style={{
                ...
                {t === 'unassigned' ? `Unassigned (${unassignedSorted.length})` : t === 'assigned' ? `Assigned (${myAssigned.length})` : `Collected (${myCollected.length})`}
              </button>
            ))}
          </div>
                ...
                <tbody>
                  {list.length === 0 ? (
                    <tr><td colSpan={6} style={{ textAlign: 'center', padding: '32px', color: 'var(--text-muted)' }}>
                      {tab === 'unassigned' ? 'No unassigned DR' : agent ? `No ${tab === 'assigned' ? 'assigned' : 'collected'} DR for ${agent.name}` : 'Select agent'}
                    </td></tr>
      ...
      {viewAll && (
        <ViewAllModal
          title={tab === 'unassigned' ? `Unassigned — ${list.length}` : `${agent?.name ?? ''} · ${tab === 'assigned' ? 'Assigned' : 'Collected'} — ${list.length}`}
      ...
      {collectTarget && (
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setCollectTarget(null); }}>
          <div className="confirm-modal" style={{ width: 380 }}>
            <div className="confirm-title">Add a remark?</div>
            <div className="confirm-body">
               <div style={{ background: 'var(--border-light)', borderRadius: 'var(--r-md)', padding: '12px 14px', marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--orange)', marginTop: 4 }}>{formatINR(collectTarget.collectedAmount ?? collectTarget.amountToCollect)}</div>
                <div style={{ fontSize: 12, color: 'var(--text-muted)', fontStyle: 'italic', marginTop: 2 }}>Already collected — this only adds a note.</div>
              </div>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text-secondary)', cursor: 'pointer' }}>
                <input type="checkbox" checked={skipRemarkPref} onChange={async e => {
                  const v = e.target.checked;
                  setSkipRemarkPref(v);
                  await db.settings.put({ key: 'wasuliSkipRemarkPrompt', value: v ? 'true' : 'false' });
                }} /> Don't ask for remark every time
              </label>
            </div>
            <div className="confirm-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => setCollectTarget(null)}>Skip</button>
              <button className="btn btn-primary btn-sm" onClick={saveRemark}>Save remark</button>
            </div>
          </div>
        </div>
      )}
