  ...
  const generate = async () => {
    ...
    if (active === 'firm') {
      ...
      setFirmSummary({
        ...
        companyName: cn?.value || 'Sundeep Freight Movers',
                ...
                <>
                  ...
                  {totalPages > 1 && (
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center', justifyContent: 'center', marginTop: 12 }}>
                      <button className="btn btn-ghost btn-sm" disabled={page === 0} onClick={() => setPage(p => p - 1)}>← Pichla</button>
                      <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Page {page + 1} of {totalPages}</span>
                      <button className="btn btn-ghost btn-sm" disabled={page === totalPages - 1} onClick={() => setPage(p => p + 1)}>Agla →</button>
                    </div>
                  )}
                </>
              )}
...
function FirmSummaryView({ data, adjustments, onAdjustmentsChange, overrides, onOverridesChange, onPrint, onPdf }: {
  ...
  return (
    <div>
      ...
      {/* Pager — same labels as generic report (Pichla/Agla) */}
      {totalPages > 1 && (
