...
export function buildWasuliReport(
  ...
  const buildGroup = (agentId: number, agentName: string, agentCode: string): ReportGroup => {
    // Oldest-first by the real action time (collectedAt/assignedAt); falls back to the
    ...
    const mine = rows.filter(r => (r.operatorId || 0) === agentId && matches(r) && inRange(eventDate(r), range))
      .sort((x, y) => {
        const tx = x.collectedAt || x.assignedAt || x.collectedDate || x.assignedDate || '';
        const ty = y.collectedAt || y.assignedAt || y.collectedDate || y.assignedDate || '';
...
const TYPE_LABEL: Record<ReportType, string> = {
  assigned: 'Assigned', pending: 'Unassigned', collected: 'Collected', all: 'All',
};
const STATUS_LABEL: Record<RowStatus, string> = { assigned: 'Assigned', pending: 'Unassigned', collected: 'Collected' };
