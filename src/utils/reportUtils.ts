...
// Compact form for counters that only grow (sidebar backlog badges) — never
// exceeds ~4 chars, so it can't push a fixed-width badge past its container.
// Locale pinned to 'en-IN' (matches formatINR) so it reads 1.2L not 120K,
// and so it's identical on every machine regardless of OS locale.
...
export async function generateMemoPDF(data: MemoPdfData): Promise<ExportResult> {
  ...
  autoTable(doc, {
    head: [['Consignor', 'Consignee', 'LR / BS No.', 'Particulars', 'Pkgs', 'ToPay', 'Paid', 'PF', 'Total', 'Collection']],
    body: data.rows.map((r) => [
      r.consignor, r.consignee, r.drNumber && r.drNumber !== '—' ? `${r.lrNumber} / ${r.drNumber}` : r.lrNumber, r.particulars,
