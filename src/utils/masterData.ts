...
// Groups challans by firm (case-insensitive, trimmed) + challan number (exact,
// trimmed). Only groups of 2+ are returned. The group's `mergeable` flag is true
// when every member is status='open' (the pre-save merge precondition).

export interface DuplicateChallanMember {
  id: number;
  status: string;
  entryCount: number;  // live (active, not deleted) entries only
  srRange: string;     // e.g. "1–7" or "—" if no entries
  srMin: number | null;
  srMax: number;
  createdSeat: string;
  truckNumber: string;
...
export interface DuplicateChallanGroup {
  firmName: string;
  challanNumber: string;
  members: DuplicateChallanMember[];
  mergeable: boolean; // true when every member is 'open' and no collected wasuli
  blockedReason?: 'not-open' | 'has-collected';
}

export async function findDuplicateChallans(): Promise<DuplicateChallanGroup[]> {
  const challans = await db.challans.filter(c => !c.deletedAt).toArray();
  const entries  = await db.lrEntries.filter(e => e.status === 'active' && !e.deletedAt).toArray();
  const wasuli   = await db.wasuli.filter(w => !w.deletedAt).toArray();

  // Group challans by canonical key
  const byKey = new Map<string, typeof challans>();
  for (const c of challans) {
    const key = c.firmName.trim().toLowerCase() + '|' + c.challanNumber.trim();
    if (!byKey.has(key)) byKey.set(key, []);
    byKey.get(key)!.push(c);
  }

  const groups: DuplicateChallanGroup[] = [];
  for (const members of byKey.values()) {
    if (members.length < 2) continue;

    const allOpen = members.every(m => m.status === 'open');
    const memberIds = new Set(members.map(m => m.id!));
    const hasCollected = wasuli.some(w => memberIds.has(w.challanId) && w.status === 'collected');
    const mergeable = allOpen && !hasCollected;
    const blockedReason = !allOpen ? 'not-open' : (hasCollected ? 'has-collected' : undefined);

    const result: DuplicateChallanGroup = {
      firmName: members[0].firmName,
      challanNumber: members[0].challanNumber,
      mergeable,
      blockedReason,
      members: members.map(c => {
        const live = entries.filter(e => e.challanId === c.id!);
        const srNos = live.map(e => e.srNo).sort((a, b) => a - b);
        const srMin = srNos.length ? srNos[0] : null;
        const srMax = srNos.length ? srNos[srNos.length - 1] : 0;
        return {
          id: c.id!,
          status: c.status,
          entryCount: live.length,
          srRange: srNos.length ? `${srNos[0]}–${srNos[srNos.length - 1]}` : '—',
          srMin,
          srMax,
          createdSeat: (c as any).seat || '',
          truckNumber: c.truckNumber || '',
          loadingStation: c.loadingStation || '',
          loadingDate: c.loadingDate || '',
          truckHire: (c as any).truckHire || 0,
        };
      }),
    };
    groups.push(result);
