...
export interface Wasuli {
  ...
  assignedDate: string;
  assignedAt?: string;   // ISO datetime — when the assign action actually happened, for true chronological ordering
  status: 'pending' | 'collected' | 'cancelled';
  collectedDate?: string;
  collectedAt?: string;  // ISO datetime — when the collect action actually happened, for true chronological ordering
...
export interface FirmLedger {
  ...
  entryDate: string;
  type: 'debit' | 'credit';
  category: 'freight' | 'delivery_charges' | 'truck_hire' | 'pf' | 'crossing' | 'door_delivery' | 'refund' | 'adjustment' | 'settlement';
  amount: number;
  description: string;
...
export interface AuditEntry {
  id?: number;
  action: 'assign' | 'reassign' | 'remove' | 'undo' | 'collect' | 'delete' | 'edit' | 'create' | 'save' | 'merge';
  ...
  detail?: string;       // freeform extra: for 'edit', a "field ₹old→₹new" summary;
                          // for 'merge', the pre-merge backup file path
}
...
export async function getAuditLog(opts: { action?: AuditEntry['action']; fromISO?: string; toISO?: string } = {}): Promise<AuditEntry[]> {
  ...
  // lazy 60-day trim (non-blocking bulk delete)
...
// Settlement model (per firm), matching the Firm Report waterfall exactly
// (computeFirmSettlement in utils/firmSettlement.ts):
//   balance = Σ[ ToPay  −  Delivery Charges  −  Truck Hire  −  Crossing  −  Door Delivery  −  Refund  +  PF ]
//   Paid is excluded — that freight was already settled at origin, never ours to hold.
