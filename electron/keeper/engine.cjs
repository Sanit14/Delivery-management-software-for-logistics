...
function mergeChallansWrite(keepId, removeId, seat) {
  const prerestoreBackupPath = writeBackupTo(getBackupFolder(), 'prerestore');
  if (!prerestoreBackupPath) {
  ...
  const result = db.transaction(() => {
    // Auto-reopen either side that's already saved — same path "Reopen memo"
    // already uses (clears its firm-ledger rows, leaves BS numbers/DRs/wasuli
    // untouched). mergeChallans re-runs eligibility itself as the authoritative
    // gate, so a challan with collected cash still refuses even after this.
    for (const id of [keepId, removeId]) {
      const c = get('challans', id);
      if (c && c.status !== 'open') cascadeReopenChallanSync(id);
    }
    const r = mergeMod.mergeChallans(mergeEngineApi, keepId, removeId);
    // The merge always leaves the winner 'open' (that's how the reopen above
    // clears its stale ledger) — auto-save it straight back to 'saved' so the
    // operator never sees an intermediate draft state. Same call ChallanDetail's
    // own Save Memo button makes; regenerates DRs for the newly-merged-in
    // entries and rebuilds the ledger (see the adjustment-row guard fix above).
    if (r.ok) memoSaveMod.saveMemo(memoEngineApi, keepId, seat);
    return r;
  })();
  dataGeneration++;
  return { ...result, backupPath: prerestoreBackupPath };
}
